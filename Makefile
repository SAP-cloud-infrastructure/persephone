...
build-all: build/scikube build/persephone-operator build/persephone-webhook build/persephone-liquid-apiserver build/persephone-audit-webhook

build/scikube: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=scikube -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/scikube ./cmd/scikube

build/persephone-operator: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=persephone-operator -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/persephone-operator ./cmd/persephone-operator

build/persephone-webhook: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=persephone-webhook -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/persephone-webhook ./cmd/persephone-webhook

build/persephone-liquid-apiserver: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=persephone-liquid-apiserver -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/persephone-liquid-apiserver ./cmd/persephone-liquid-apiserver

build/persephone-audit-webhook: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=persephone-audit-webhook -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/persephone-audit-webhook ./cmd/persephone-audit-webhook
...
install: FORCE build/scikube build/persephone-operator build/persephone-webhook build/persephone-liquid-apiserver build/persephone-audit-webhook
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/scikube "$(DESTDIR)$(PREFIX)/bin/scikube"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/persephone-operator "$(DESTDIR)$(PREFIX)/bin/persephone-operator"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/persephone-webhook "$(DESTDIR)$(PREFIX)/bin/persephone-webhook"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/persephone-liquid-apiserver "$(DESTDIR)$(PREFIX)/bin/persephone-liquid-apiserver"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/persephone-audit-webhook "$(DESTDIR)$(PREFIX)/bin/persephone-audit-webhook"

# which packages to test with test runner
GO_TESTPKGS := $(shell go list -f '{{if or .TestGoFiles .XTestGoFiles}}{{.Dir}}{{end}}' ./... | grep -Ev 'test/integration')
ifeq ($(GO_TESTPKGS),)
GO_TESTPKGS := ./...
endif
...
run-shellcheck: FORCE install-shellcheck
	...
	@find . \( -path '*/_scratch/*/*' -prune \) -o \( -path '_scratch/*' -prune \) -o -type f \( -name '*.bash' -o -name '*.ksh' -o -name '*.zsh' -o -name '*.sh' -o -name '*.shlib' \) -exec shellcheck --external-sources {} +
...
build/cover.out: FORCE | build
	...
	@env $(GO_TESTENV) go run github.com/onsi/ginkgo/v2/ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=persephone -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTFLAGS) $(GO_TESTPKGS)
...
license-headers: FORCE install-addlicense install-reuse
	...
	@printf "%s\0" $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...)) | $(XARGS) -0 -I{} bash -c 'year="$$(grep 'Copyright' {} | head -n1 | grep -E -o '"'"'[0-9]{4}(-[0-9]{4})?'"'"')"; if [[ -z "$$year" ]]; then year=$$(date +%Y); fi; gawk -i inplace '"'"'{if (display) {print} else {!/^\/\*/ && !/^\*/}}; {if (!display && $$0 ~ /^(package |$$)/) {display=1} else { }}'"'"' {}; addlicense -c "SAP SE or an SAP affiliate company and Project Persephone contributors" -s=only -y "$$year" -- {}; $(SED) -i '"'"'1s+// Copyright +// SPDX-FileCopyrightText: +'"'"' {}; '
	@printf "\e[1;36m>> reuse annotate (for license headers on other files)\e[0m\n"
	@reuse lint -j | jq -r '.non_compliant.missing_licensing_info[]' | sed '/\<vendor\>/d' | $(XARGS) reuse annotate -c 'SAP SE or an SAP affiliate company and Project Persephone contributors' -l Apache-2.0 --skip-unrecognised
...
goimports: FORCE install-goimports
	...
	@goimports -w -local github.com/sap-cloud-infrastructure/persephone $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/scikube\e[0m                      Build scikube.\n"
	@printf "  \e[36mbuild/persephone-operator\e[0m          Build persephone-operator.\n"
	@printf "  \e[36mbuild/persephone-webhook\e[0m           Build persephone-webhook.\n"
	@printf "  \e[36mbuild/persephone-liquid-apiserver\e[0m  Build persephone-liquid-apiserver.\n"
	@printf "  \e[36mbuild/persephone-audit-webhook\e[0m     Build persephone-audit-webhook.\n"
